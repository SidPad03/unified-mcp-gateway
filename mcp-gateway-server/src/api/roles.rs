...
async fn update_role(
    ...
    let role: Option<(bool,)> = sqlx::query_as("SELECT is_system FROM roles WHERE role_id = $1")
        .bind(id)
...
async fn role_impact(
    ...
    let role: Option<(String, bool)> =
        sqlx::query_as("SELECT name, is_system FROM roles WHERE role_id = $1")
            .bind(id)
            .fetch_optional(&state.db)
            .await?;
    ...
    let policy_count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM role_policies WHERE role_id = $1")
            .bind(id)
            .fetch_one(&state.db)
            .await?;

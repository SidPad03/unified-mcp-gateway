...
```
 ...
 agent (Rust TUI) ──/agent/ws + key▶│                            PostgreSQL (users, backends,
...
| **Agent** | `mcp-gateway-agent/` | Runs on a user's machine, dials **out** to `/agent/ws`, and bridges that machine's local MCP servers into the gateway. |

The agent↔server wire protocol is currently defined in both crates; extracting
it into a shared crate is a known follow-up.
...
| `agent` | The backend lives on a remote machine running `mcp-gateway-agent`, reached over the WebSocket that agent opened. See [Agent Architecture](agent-architecture.md). |

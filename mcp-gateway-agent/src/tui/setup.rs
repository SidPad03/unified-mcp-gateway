...
impl SetupWizard {
    ...
    pub fn handle_event(&mut self, event: &Event) {
        ...
        match self.screen {
            ...
            Screen::Backends => match key.code {
                KeyCode::Char('a') => {
                    ...
                    if let Some(idx) = self.backend_list_state.selected() {
                        if idx < self.backends.len() {
                            ...
                            if !self.backends.is_empty() && idx >= self.backends.len() {
                                self.backend_list_state
                                    .select(Some(self.backends.len() - 1));
                            }
                        }
                    }
                }
            ...
            Screen::AddBackendName => match key.code {
                KeyCode::Enter => {
            ...
            Screen::AddBackendTransport => match key.code {
            ...
            Screen::AddBackendDetail => match self.add_transport {
    ...
    fn validate_api_key(&mut self) {
        ...
        let url = format!(
            "{}?token={}",
            self.gateway_input.value, self.api_key_input.value
        );
        let tx = self.validation_tx.clone();
        tokio::spawn(async move {
    ...
    fn render_welcome(&self, f: &mut Frame, area: Rect) {
        ...
        let logo = Paragraph::new(theme::ASCII_LOGO).style(theme::title_style());
    ...
    fn render_gateway(&mut self, f: &mut Frame, area: Rect) {
        ...
        let validation = Line::from(vec![super::widgets::validation_indicator(
            self.gateway_validation,
        )]);
        f.render_widget(Paragraph::new(validation), chunks[3]);

        let hint2 = Paragraph::new(
            "Dashboard URL for updates (leave empty to auto-derive from gateway URL)",
        )
        .style(theme::dim());
        ...
        let tls_hint = Paragraph::new(tls_label).style(if self.tls_skip_verify {
            theme::status_warn()
        } else {
            theme::dim()
        });
    ...
    fn render_backends(&mut self, f: &mut Frame, area: Rect) {
        ...
        let hint =
            Paragraph::new("  a:add  d:delete  Enter:continue  Esc:back").style(theme::dim());
        f.render_widget(hint, chunks[3]);
    }
    ...
    fn render_add_backend_name(&mut self, f: &mut Frame, area: Rect) {
        ...
        let header = Paragraph::new("  Add Backend - Name").style(theme::title_style());
    ...
    fn render_add_backend_transport(&self, f: &mut Frame, area: Rect) {
        ...
        let header = Paragraph::new("  Add Backend - Transport").style(theme::title_style());
        f.render_widget(header, chunks[0]);

        let hint = Paragraph::new("Select the transport type:").style(theme::dim());
        f.render_widget(hint, chunks[1]);
        ...
        let options = Paragraph::new(vec![
            ...
            Line::from(Span::styled(
                if self.add_transport == TransportChoice::Stdio {
            ...
            Line::from(""),
            Line::from(Span::styled(
                if self.add_transport == TransportChoice::Http {
                    "  > [2] http   - Connect to HTTP endpoint"
                } else {
                    "    [2] http   - Connect to HTTP endpoint"
                },
                http_style,
    ...
    fn render_add_backend_detail(&mut self, f: &mut Frame, area: Rect) {
        ...
        let header = Paragraph::new("  Add Backend - Details").style(theme::title_style());
        f.render_widget(header, chunks[0]);
        ...
        match self.add_transport {
            ...
            TransportChoice::Http => {
                let hint =
                    Paragraph::new("Enter the URL for the HTTP MCP backend").style(theme::dim());
                f.render_widget(hint, chunks[1]);
    ...
    fn render_summary(&self, f: &mut Frame, area: Rect) {
        ...
        let hint = Paragraph::new("  Enter:save  Esc:back").style(theme::dim());
        f.render_widget(hint, chunks[3]);
    }
    ...
    fn render_done(&self, f: &mut Frame, area: Rect) {
        ...
        let options = Paragraph::new(vec![
            ...
            Line::from(Span::styled(
                "  [s] Install as a background service (launchd)",
                theme::normal(),
            )),
